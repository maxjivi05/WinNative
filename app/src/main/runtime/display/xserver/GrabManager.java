...
public class GrabManager implements WindowManager.OnWindowModificationListener {
  ...
  private void activatePointerGrab(
      Window window, EventListener eventListener, boolean ownerEvents, boolean releaseWithButtons) {
    if (this.window == null) {
      xServer.inputDeviceManager.sendEnterLeaveNotify(
  ...
  public void activatePointerGrab(Window window) {
    ...
    activatePointerGrab(
        window, eventListener, eventListener.isInterestedIn(Event.OWNER_GRAB_BUTTON), true);
  }
}
