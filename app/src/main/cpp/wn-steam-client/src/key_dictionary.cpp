// Valve's RSA-1024 public keys per EUniverse, DER-encoded SubjectPublicKeyInfo
// form. Bytes mirror SteamKit2's KeyDictionary.cs (master, fetched 2026-05-15)
// and JavaSteam's KeyDictionary.java. Each blob:
//   - is exactly 160 bytes,
//   - has the rsaEncryption AlgorithmIdentifier SPKI header
//     (30 81 9D 30 0D 06 09 2A 86 48 86 F7 0D 01 01 01 05 00 03 81 8B 00
//      30 81 87 02 81 81 00),
//   - encodes a 128-byte modulus,
//   - has public exponent 0x11 (17).
//
// Padding for ChannelEncryptResponse is RSA-OAEP with SHA-1 (both MGF1 and
// label hash). See crypto.cpp; the encrypted-channel handshake layer that
// consumes these keys lands in Phase 1E.
...
constexpr std::array<uint8_t, 160> kPublicUniverseKey = {
...
constexpr std::array<uint8_t, 160> kBetaUniverseKey = {
    0x30, 0x81, 0x9D, 0x30, 0x0D, 0x06, 0x09, 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x01, 0x01, 0x01,
...
constexpr std::array<uint8_t, 160> kInternalUniverseKey = {
    0x30, 0x81, 0x9D, 0x30, 0x0D, 0x06, 0x09, 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x01, 0x01, 0x01,
...
constexpr std::array<uint8_t, 160> kDevUniverseKey = {
    0x30, 0x81, 0x9D, 0x30, 0x0D, 0x06, 0x09, 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x01, 0x01, 0x01,
...
std::span<const uint8_t> get_universe_public_key(EUniverse universe) noexcept {
    switch (universe) {
        case EUniverse::Public:   return {kPublicUniverseKey.data(),   kPublicUniverseKey.size()};
        case EUniverse::Beta:     return {kBetaUniverseKey.data(),     kBetaUniverseKey.size()};
        case EUniverse::Internal: return {kInternalUniverseKey.data(), kInternalUniverseKey.size()};
        case EUniverse::Dev:      return {kDevUniverseKey.data(),      kDevUniverseKey.size()};
        case EUniverse::Invalid:
        case EUniverse::Max:
        default:
            return {};
    }
