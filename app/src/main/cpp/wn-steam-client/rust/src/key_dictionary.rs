use crate::emsg::EUniverse;

pub const PUBLIC_UNIVERSE_KEY: [u8; 160] = [
...
pub const BETA_UNIVERSE_KEY: [u8; 160] = [
    0x30, 0x81, 0x9D, 0x30, 0x0D, 0x06, 0x09, 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x01, 0x01, 0x01,
...
pub const INTERNAL_UNIVERSE_KEY: [u8; 160] = [
    0x30, 0x81, 0x9D, 0x30, 0x0D, 0x06, 0x09, 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x01, 0x01, 0x01,
...
pub const DEV_UNIVERSE_KEY: [u8; 160] = [
    0x30, 0x81, 0x9D, 0x30, 0x0D, 0x06, 0x09, 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x01, 0x01, 0x01,
...
pub fn get_universe_public_key(universe: EUniverse) -> &'static [u8] {
    match universe {
        EUniverse::Public => &PUBLIC_UNIVERSE_KEY,
        EUniverse::Beta => &BETA_UNIVERSE_KEY,
        EUniverse::Internal => &INTERNAL_UNIVERSE_KEY,
        EUniverse::Dev => &DEV_UNIVERSE_KEY,
        EUniverse::Invalid | EUniverse::Max => &[],
    }
}
...
mod tests {
    ...
    #[test]
    fn returns_keys_for_known_universes_only() {
        assert_eq!(get_universe_public_key(EUniverse::Public).len(), 160);
        assert_eq!(get_universe_public_key(EUniverse::Beta).len(), 160);
        assert_eq!(get_universe_public_key(EUniverse::Internal).len(), 160);
        assert_eq!(get_universe_public_key(EUniverse::Dev).len(), 160);
        assert!(get_universe_public_key(EUniverse::Invalid).is_empty());
    }

    #[test]
    fn keys_are_spki_rsa_with_exponent_17() {
        for universe in [
            EUniverse::Public,
            EUniverse::Beta,
            EUniverse::Internal,
            ...
            let key = get_universe_public_key(universe);
            assert_eq!(&key[..8], &[0x30, 0x81, 0x9D, 0x30, 0x0D, 0x06, 0x09, 0x2A]);
            assert_eq!(&key[157..], &[0x02, 0x01, 0x11]);
        }
    }
}
