...
static int getBitmapBytePad(int width) { return ((width + 32 - 1) >> 5) << 2; }
...
Java_com_winlator_cmod_runtime_display_xserver_Drawable_drawBitmap(
    JNIEnv *env, jclass obj, jshort width, jshort height, jobject srcData,
    jobject dstData) {
...
JNIEXPORT void JNICALL
Java_com_winlator_cmod_runtime_display_xserver_Drawable_copyArea(
    JNIEnv *env, jclass obj, jshort srcX, jshort srcY, jshort dstX, jshort dstY,
    jshort width, jshort height, jshort srcStride, jshort dstStride,
...
JNIEXPORT void JNICALL
Java_com_winlator_cmod_runtime_display_xserver_Drawable_copyAreaOp(
    JNIEnv *env, jclass obj, jshort srcX, jshort srcY, jshort dstX, jshort dstY,
    jshort width, jshort height, jshort srcStride, jshort dstStride,
    jobject srcData, jobject dstData, int gcFunction) {
  uint8_t *srcDataAddr = (*env)->GetDirectBufferAddress(env, srcData);
  uint8_t *dstDataAddr = (*env)->GetDirectBufferAddress(env, dstData);
  ...
  for (int16_t y = 0; y < height; y++) {
    for (int16_t x = 0; x < width; x++) {
      ...
      int srcColor = (srcDataAddr[i] << 16) | (srcDataAddr[i + 1] << 8) |
                     srcDataAddr[i + 2];
      int dstColor = (dstDataAddr[j] << 16) | (dstDataAddr[j + 1] << 8) |
                     dstDataAddr[j + 2];
...
JNIEXPORT void JNICALL
Java_com_winlator_cmod_runtime_display_xserver_Drawable_fillRect(
    JNIEnv *env, jclass obj, jshort x, jshort y, jshort width, jshort height,
    jint color, jshort stride, jobject data) {
...
JNIEXPORT void JNICALL
Java_com_winlator_cmod_runtime_display_xserver_Drawable_drawLine(
    JNIEnv *env, jclass obj, jshort x0, jshort y0, jshort x1, jshort y1,
    jint color, jshort lineWidth, jshort stride, jobject data) {
  uint8_t *dataAddr = (*env)->GetDirectBufferAddress(env, data);
...
JNIEXPORT void JNICALL
Java_com_winlator_cmod_runtime_display_xserver_Drawable_drawAlphaMaskedBitmap(
    JNIEnv *env, jclass obj, jbyte foreRed, jbyte foreGreen, jbyte foreBlue,
    jbyte backRed, jbyte backGreen, jbyte backBlue, jobject srcData,
    jobject maskData, jobject dstData) {
  ...
  for (int i = 0; i < dstLength; i++) {
    dstDataAddr[i] =
        maskDataAddr[i] == WHITE
            ? (srcDataAddr[i] == WHITE ? foreColor : backColor) | 0xff000000
            : 0x00000000;
  }
}

JNIEXPORT void JNICALL
Java_com_winlator_cmod_runtime_display_xserver_Drawable_fromBitmap(
    JNIEnv *env, jclass obj, jobject bitmap, jobject data) {
...
JNIEXPORT void JNICALL
Java_com_winlator_cmod_runtime_display_xserver_Pixmap_toBitmap(
    JNIEnv *env, jclass obj, jobject colorData, jobject maskData,
    jobject bitmap) {
  char *colorDataAddr = (*env)->GetDirectBufferAddress(env, colorData);
  char *maskDataAddr =
      maskData ? (*env)->GetDirectBufferAddress(env, maskData) : NULL;
