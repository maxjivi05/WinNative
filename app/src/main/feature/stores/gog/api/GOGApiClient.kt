...
class GOGApiClient
    ...
    constructor(
        ...
        suspend fun fetchDependencyRepository(url: String): Result<DependencyRepository> =
            withContext(Dispatchers.IO) {
                try {
                    val credentials = GOGAuthManager.getStoredCredentials(context).getOrNull()
         ...
         */
        suspend fun getDependencyOpenLink(): Result<List<String>> =
            withContext(Dispatchers.IO) {
                try {
                    val credentials = GOGAuthManager.getStoredCredentials(context).getOrNull()
                    if (credentials == null) {
        ...
        suspend fun fetchDependencyManifest(manifestUrl: String): Result<GOGDependencyManifestMeta> =
            withContext(Dispatchers.IO) {
                try {
                    val credentials = GOGAuthManager.getStoredCredentials(context).getOrNull()
                    if (credentials == null) {
