...
class GOGManifestParser
    ...
    constructor() {
         ...
         */
        fun filterDepotsByLanguage(
            manifest: GOGManifestMeta,
            containerLanguage: String,
        ): Pair<List<Depot>, String> {
            val allDepotLangCodes =
                manifest.depots
                ...
                if (matched.isNotEmpty()) {
                    ...
                    Timber
                        .tag(
                            ...
                            "No depots for requested language codes, fell back to $fallbackLang: ${result.size}/${manifest.depots.size} depots",
                        )
        ...
        ): Pair<List<DepotFile>, List<DepotFile>> {
         ...
         */
        fun findDLCProducts(manifest: GOGManifestMeta): List<Product> = manifest.products.filter { it.productId != manifest.baseProductId }
         ...
         */
        fun hasDLC(manifest: GOGManifestMeta): Boolean = findDLCProducts(manifest).isNotEmpty()
         ...
         */
        fun detectGeneration(build: GOGBuild): Int = build.generation

        /**
         * Parse builds response JSON
         */
        fun parseBuilds(json: String): BuildsResponse = BuildsResponse.fromJson(JSONObject(json))

        /**
         * Parse manifest metadata JSON
         */
        fun parseDependencyManifest(json: String): GOGDependencyManifestMeta = GOGDependencyManifestMeta.fromJson(JSONObject(json))
         ...
         */
        fun parseDepotManifest(json: String): DepotManifest = DepotManifest.fromJson(JSONObject(json))

        /**
         * Parse secure links response JSON
         */
        fun parseSecureLinks(json: String): SecureLinksResponse = SecureLinksResponse.fromJson(JSONObject(json))
