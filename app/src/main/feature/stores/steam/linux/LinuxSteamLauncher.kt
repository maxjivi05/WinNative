...
object LinuxSteamLauncher {
    ...
    fun prepare(
        ...
        // Truncate before tailing: tailer starts at position = length(), so
        // without reset prior-launch bytes are invisible and the file grows
        // unbounded across the script's `exec >>"$LOG"` appends. Path must
        // match `launcherLogPath` in renderLauncherScript().
        val launcherLog = File(imageFs.rootDir, "var/log/winnative-steam.log")
        runCatching {
            launcherLog.parentFile?.mkdirs()
            launcherLog.writeText("")
        }.onFailure { Log.w(TAG, "could not truncate $launcherLog: ${it.message}") }
        SteamLogTailer.start(launcherLog)
    ...
    private fun renderLauncherScript(
        ...
        // Bootstrap-suppression flags from ROCKNIX's working ARM64 launch
        // (projects/ROCKNIX/.../start_steam.sh). On every launch Steam will
        // otherwise re-run its initial-bootstrap pass: fetch package/beta,
        // hit client-update.steamstatic.com, verify every file on disk,
        // attempt repair if anything looks off. Under proot's ptrace
        // overhead each of those forks/syscalls is slow; together they
        // routinely stall arm64 Steam startup. We pre-extracted the entire
        // payload so verify+repair are pure waste anyway. Skipping them
        // also avoids the pulseaudio/dbus/xdg-utils system() calls Steam
        // makes during bootstrap, which are the proot stall sources the
        // launcher script's existing comments call out.
        val bootstrapSkipFlags = listOf(
        ...
        val steamArgs = if (steamAppId == null) {
            listOf("-console", "-tcp") + bootstrapSkipFlags
        } else {
            listOf("-silent", "-tcp") + bootstrapSkipFlags +
                listOf("-applaunch", steamAppId.toString())
        }
        ...
        val launcherLogPath = imageFs.rootDir.absolutePath + "/var/log/winnative-steam.log"
        val text = buildString {
            ...
            appendLine("            SDL_VIDEODRIVER=x11 \\")
            ...
            appendLine("        USER=xuser \\")
            appendLine("        DISPLAY=:0 \\")
            // ROCKNIX's working ARM64 launch (start_steam.sh) pins SDL to
            // x11. Sniper's bundled SDL probes wayland first, finds none
            // (we don't run wayland under proot), then falls through to
            // x11. Pinning skips the wayland probe path, which on some
            // sniper builds ships a libwayland-client that dlopen-stalls
            // its protocol-discovery thread.
            appendLine("        SDL_VIDEODRIVER=x11 \\")
            appendLine("        XDG_RUNTIME_DIR=\"\$XDG_RUNTIME_DIR_IN_PROOT\" \\")
