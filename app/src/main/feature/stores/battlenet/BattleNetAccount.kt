...
object BattleNetAccount {
    ...
    suspend fun completeSignIn(context: android.content.Context, credential: BattleNetCredential) = mutation.withLock {
        withContext(NonCancellable) {
            ...
            withContext(Dispatchers.IO) { BattleNetWebSession.clear(context) }
    ...
    suspend fun games(context: android.content.Context): List<BattleNetGame> {
        val epoch = generation.get()
        val endpoint = "$ORIGIN/api/games-and-subs"
        val browserCookie = withContext(Dispatchers.Main) { CookieManager.getInstance().getCookie(endpoint) }
        val savedCookie = withContext(Dispatchers.IO) { BattleNetWebSession.read(context) }
        val candidates = listOfNotNull(browserCookie, savedCookie).filter { it.isNotBlank() }.distinct()
        for (cookie in candidates) {
            val result = withContext(Dispatchers.IO) {
                val request = Request.Builder().url(endpoint)
                    .header("Cookie", cookie).header("Accept", "application/json").build()
                client.newCall(request).execute().use { response ->
                    if (response.code in listOf(301, 302, 303, 307, 308, 401, 403)) return@use null
                    ...
                    if (body.contentLength() > 2 * 1024 * 1024) throw IOException("Battle.net library response is too large.")
                    ...
                    val root = try { JSONObject(bytes.toString(Charsets.UTF_8)) } catch (_: Exception) { return@use null }
                    val accounts = root.optJSONArray("gameAccounts") ?: return@use null
                    ...
                    if (headers.sumOf { it.length.toLong() } > 65536) throw IOException("Invalid Battle.net session.")
                    Triple(BattleNetCatalog.ownedGames(ids), BattleNetCookieRotation.merge(cookie, headers), headers)
                }
            } ?: continue
            return mutation.withLock {
                if (epoch != generation.get()) throw SignInRequired()
                withContext(Dispatchers.IO) { BattleNetWebSession.save(context, result.second) }
                withContext(Dispatchers.Main) {
                    val manager = CookieManager.getInstance()
                    for (header in result.third) {
                        suspendCancellableCoroutine { continuation ->
                            manager.setCookie(endpoint, header) { if (continuation.isActive) continuation.resume(Unit) }
                        }
                    }
