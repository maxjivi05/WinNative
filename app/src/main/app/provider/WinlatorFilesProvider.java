package com.winlator.cmod.app.provider;
...
public class WinlatorFilesProvider extends DocumentsProvider {
  ...
  public void attachInfo(Context context, ProviderInfo info) {
    ...
    enabled =
        PreferenceManager.getDefaultSharedPreferences(context)
            .getBoolean("enable_file_provider", true);
  }
  ...
  public String moveDocument(
    ...
    if (Objects.equals(source.getParentFile(), sourceParent))
      throw new FileNotFoundException(
          "Source has wrong parent: " + sourceDocumentId + " " + sourceParentDocumentId);
    ...
    if (target.exists()) throw new FileNotFoundException("Target already exist");

    boolean ret = source.renameTo(target);
    if (!ret) throw new FileNotFoundException("Failed to move: " + sourceDocumentId);
  ...
  public void removeDocument(String documentId, String parentDocumentId)
    ...
    if (!target.exists()) throw new FileNotFoundException("File is not found: " + documentId);

    ret = target.delete();
    if (!ret) throw new FileNotFoundException("Failed to delete file: " + documentId);
  }
  ...
  private void includeFile(MatrixCursor result, String docId, File file)
      throws FileNotFoundException {
    if (!enabled) throw new FileNotFoundException();
