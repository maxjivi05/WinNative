...
Completed native installs now create first-class main Library shortcuts from the Rust installed record even when Blizzard's Agent has temporarily cleared its playable flag. Existing Battle.net shortcuts are updated in place when an update changes the shared install folder, preserving their other settings. The shortcut carries the Battle.net product, shared install path, client executable and downloaded artwork for the Library card and hero view.

The Library detail screen recognizes the shortcut as Battle.net, shows the Battle.net source button, and provides Verify Files and Check for Update through the native Rust client. Play uses BattleNetRuntime, and is disabled while verification or an update is active. Device validation created the World of Warcraft Classic Library card, rendered its artwork and 21.38 GiB installed size, completed all 384,508 verification objects from the Library menu, reported no update available, launched the shared Battle.net client from Library Play, and exited the container cleanly. The empty Change Store action is hidden when no alternate installed store exists.

The latest authenticated Library Play test restored the saved Battle.net account and reached the client. The Agent received the wow_classic launch request but kept it non-playable with error 2123 because it could not create its TACT container folder under the Android shared-storage drive. Direct authenticated game launch therefore remains separate from the completed Library integration.
